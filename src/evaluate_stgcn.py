...
from src.models.stgcn_multitask import (
    CoreSetSTGCN_MultiTask
)
...
def run_inference(
    ...
    with torch.no_grad():
        ...
        for inputs, labels, density_gts in loader:
            ...
            all_density_pred.append(
                density_maps.cpu().numpy()
            )

            all_density_gt.append(
                density_gts.cpu().numpy()
            )
...
def evaluate(
    ...
    mae, rmse, obo, pred_counts, gt_counts = \
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        ...
        description=
        "Evaluate CoreSet ST-GCN on the held-out test set."
    )
