...
Implements the full training protocol described in the methodology:
 ...
 Optimiser       : AdamW (Loshchilov & Hutter, 2019; building on Adam,
                   Kingma & Ba, 2015). AdamW correctly decouples weight-decay
                   regularisation from adaptive gradient scaling, producing
                   better generalisation than the standard Adam formulation.
 ...
 Dropout         : 0.5 before classification head, 0.3 before density head,
                   per the methodology Training Protocol.
...
def train_stgcn_model(config_path: str = 'configs/stgcn_config.yaml'):
   ...
   #                                                                      #
   #  FIX (Issue 4): Normalization stats are computed and stored as       #
   #  tensors of shape (1, C, 1, 1, 1) so they broadcast correctly        #
   #  against model input tensors of shape (B, C, T, V, 1).              #
   #  The dataset returns angle_seq of shape (T, C); the DataLoader       #
   #  stacks these into (B, T, C) which the dataset class then            #
   #  permutes/reshapes to (B, C, T, V, 1) before returning.             #
   #  Collapsing axes (0, 2, 3, 4) — i.e., batch, time, node, M —        #
   #  yields one mean and one std per channel (angle type), which is      #
   #  the intended per-channel normalization described in the methodology. #
   # ------------------------------------------------------------------ #
   ...
   # Collapse all axes except the channel axis (dim=1) to obtain
   # per-channel statistics; keepdim=True preserves (1, C, 1, 1, 1)
   # shape for correct broadcasting against (B, C, T, V, 1) inputs.
   feat_mean = all_train_features.mean(dim=(0, 2, 3, 4), keepdim=True)  # (1, C, 1, 1, 1)
   feat_std  = all_train_features.std(dim=(0, 2, 3, 4), keepdim=True).clamp(min=1e-6)  # (1, C, 1, 1, 1)

   # Sanity-check: shapes must be (1, C, 1, 1, 1)
   assert feat_mean.shape == (1, all_train_features.shape[1], 1, 1, 1), (
       f"Unexpected feat_mean shape {feat_mean.shape}; "
       "normalization will not broadcast correctly."
   )

   print(f"  Feature mean shape : {tuple(feat_mean.shape)}")
   print(f"  Feature std  shape : {tuple(feat_std.shape)}")
   ...
   #  (Loshchilov & Hutter, 2019; building on Adam, Kingma & Ba, 2015)    #
   #  "weight_decay = 1e-4" per the methodology Training Protocol.        #
   # ------------------------------------------------------------------ #
   ...
   for epoch in range(config['epochs']):
       ...
       for inputs, labels, density_gts in train_loader:
           # NOTE (Issue 4): feat_mean/feat_std have shape (1, C, 1, 1, 1)
           # and inputs have shape (B, C, T, V, 1); broadcasting is correct.
           inputs      = inputs.to(device)
           inputs      = (inputs - feat_mean.to(device)) / feat_std.to(device)

           # FIX (Issue 5): Gaussian noise is injected HERE, after Z-score,
           # so the perturbation magnitude (±0.05) is always relative to one
           # standardised unit — a controlled fraction of one standard
           # deviation, independent of the raw angle scale.
           # Noise is only applied during training, never during val/test.
       ...
       if avg_val_loss < best_val_loss:
           ...
           torch.save({
               ...
               'config':               config,
               'feat_mean':            feat_mean,   # shape (1, C, 1, 1, 1)
               'feat_std':             feat_std,    # shape (1, C, 1, 1, 1)
           }, checkpoint_path)
   ...
   # Reload normalization stats from checkpoint to guarantee consistency
   # between training and any future inference session that loads this file.
